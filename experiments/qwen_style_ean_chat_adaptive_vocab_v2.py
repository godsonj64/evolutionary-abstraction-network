...
"""Cleaner adaptive-vocabulary Qwen-tokenized EAN chat pipeline.

This v2 keeps Qwen tokenization but avoids two failure modes observed in quick
Colab runs:

1. training a huge 151k output head from tiny data;
2. leaking Qwen internal special tokens such as <|im_end|> during generation.

The script remaps observed Qwen token ids to a compact local vocabulary, encodes
training text without automatic Qwen chat-template special tokens, appends EOS
manually, and bans most special/control tokens during generation.

Run:
    python experiments/qwen_style_ean_chat_adaptive_vocab_v2.py --device cuda --quick
"""
...
class AdaptiveQwenTokenizerV2:
    def __init__(self, model_name: str, texts: list[str], max_local_vocab: int):
        ...
        for token in [self.base.pad_token, self.base.eos_token, self.base.bos_token, self.base.unk_token] + EAN_SPECIALS:
            if token is None:
                continue
            tid = self.base.convert_tokens_to_ids(token)
            if tid is not None and tid != -1:
                required_global_ids.add(int(tid))

        counts = Counter()
        for text in texts:
            counts.update(self.base.encode(text, add_special_tokens=False))
        ordered = list(required_global_ids)
        ...
        # Ban all special/control tokens during free generation except EOS. The prompt can still contain them.
    ...
    def state(self) -> dict:
        return {"base_tokenizer": self.loaded_name, "adaptive_vocab_size": self.vocab_size, "pad_id": self.pad_id, "eos_id": self.eos_id, "banned_generation_ids": len(self.banned_generation_ids), "ean_specials": EAN_SPECIALS}


class LMDataset(Dataset):
    def __init__(self, texts: list[str], tok: AdaptiveQwenTokenizerV2, block_size: int):
    ...
    def __getitem__(self, idx):
        ...
        target = target.masked_fill(target == self.pad_id, -100)
        return torch.tensor(ids[:-1], dtype=torch.long), target
...
@torch.no_grad()
def generate(model, tok, prompt, device, max_new=100, temperature=0.75, top_k=40, repetition_penalty=1.25):
    ...
    for _ in range(max_new):
        ...
        for bid in tok.banned_generation_ids:
            if bid != tok.eos_id and 0 <= bid < logits.size(-1): logits[:, bid] = -float("inf")
        for rid in set(int(i) for i in x[0, -64:].tolist()):
...
def main():
    ...
    p.add_argument("--tokenizer-name", default="Qwen/Qwen3-0.6B"); p.add_argument("--adaptive-vocab-size", type=int, default=8000)
    p.add_argument("--block-size", type=int, default=128); p.add_argument("--batch-size", type=int, default=32)
    p.add_argument("--pretrain-epochs", type=int, default=5); p.add_argument("--instruct-epochs", type=int, default=5); p.add_argument("--chat-epochs", type=int, default=5)
    ...
    p.add_argument("--seed", type=int, default=42); p.add_argument("--output-dir", default="outputs/qwen_style_ean_chat_adaptive_vocab_v2")
