...
"""Qwen-tokenized EAN chat model with robust prompt-answer SFT.

v5 fixes the v4 chat-dataset failure by representing instruction/chat examples
as explicit (prompt, answer) pairs. The model sees the prompt tokens, but loss is
applied only to answer tokens. This avoids brittle marker scanning and works with
Alpaca and UltraChat message schemas.

Run:
    python experiments/qwen_style_ean_chat_sft_v5.py --device cuda
"""
...
class AdaptiveQwenTokenizer:
    def __init__(self, model_name: str, corpus_texts: list[str], max_local_vocab: int):
        ...
        for gid, _ in counts.most_common(max_local_vocab * 4):
            gid = int(gid)
    ...
    def decode(self, ids: list[int], skip_control: bool = True) -> str:
        ...
        for lid in ids:
            ...
            if lid == self.eos_id:
                break
        return re.sub(r"\s+", " ", self.base.decode(gids, skip_special_tokens=skip_control)).strip()

    def state(self) -> dict:
        return {"base_tokenizer": self.loaded_name, "adaptive_vocab_size": self.vocab_size, "pad_id": self.pad_id, "eos_id": self.eos_id, "special_local": self.special_local, "banned_generation_ids": len(self.banned_generation_ids)}
...
class AssistantPairDataset(Dataset):
    def __init__(self, pairs: list[tuple[str, str]], tok: AdaptiveQwenTokenizer, block_size: int):
        self.samples, self.pad_id, self.block_size = [], tok.pad_id, block_size
        for prompt, answer in pairs:
            p_ids = tok.encode(prompt, append_eos=False)
            a_ids = tok.encode(answer + " <|end|>", append_eos=True)
            if len(a_ids) < 2:
                continue
            # Keep as much answer as possible and left-truncate long prompts.
            max_answer = max(8, block_size // 2)
            if len(a_ids) > max_answer:
                a_ids = a_ids[:max_answer - 1] + [tok.eos_id]
            room_for_prompt = block_size + 1 - len(a_ids)
            if room_for_prompt < 2:
                continue
            p_ids = p_ids[-room_for_prompt:]
            ids = p_ids + a_ids
            labels = [-100] * len(p_ids) + a_ids
            if len(ids) < 4 or sum(v != -100 for v in labels) < 2:
                continue
            self.samples.append((ids, labels))
        if not self.samples:
            raise ValueError("No valid assistant pair SFT samples built.")
...
def load_instruct_pairs(n: int) -> list[tuple[str, str]]:
    ...
    for r in ds:
        ...
        prompt = f"<|user|> {user} <|assistant|> <|answer|>"
        pairs.append((prompt, out))
    return pairs[:n]


def load_chat_pairs(n: int) -> list[tuple[str, str]]:
    ...
    for r in ds:
        ...
        for m in messages:
            ...
            if role == "assistant":
                if context:
                    prompt = " ".join(context) + " <|assistant|> <|answer|>"
...
@torch.no_grad()
def generate(model, tok, prompt, device, min_new=64, max_new=180, temperature=0.72, top_k=40, repetition_penalty=1.55):
    ...
    for step in range(max_new):
        ...
        if step < min_new and 0 <= tok.eos_id < logits.size(-1): logits[:, tok.eos_id] = -float("inf")
        for rid in set(int(i) for i in x[0, -160:].tolist()):
...
def main():
    ...
    p.add_argument("--repetition-penalty", type=float, default=1.55)
    ...
    p.add_argument("--output-dir", default="outputs/qwen_style_ean_chat_sft_v5")
    args = p.parse_args(); set_seed(args.seed)

    device = torch.device(args.device if args.device != "cuda" or torch.cuda.is_available() else "cpu")
    outdir = Path(args.output_dir); outdir.mkdir(parents=True, exist_ok=True)
    pre = load_pretrain(args.pretrain_samples)
    instruct_pairs = load_instruct_pairs(args.instruct_samples)
    chat_pairs = load_chat_pairs(args.chat_samples)
    corpus = pre + [p + " " + a for p, a in instruct_pairs] + [p + " " + a for p, a in chat_pairs]
    tok = AdaptiveQwenTokenizer(args.tokenizer_name, corpus, args.adaptive_vocab_size)
    cfg = ChatEANConfig(tok.vocab_size, tok.pad_id, args.block_size)
    model = EANChatLM(cfg).to(device); configure_evolution(model, args)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    print(json.dumps({"device": str(device), "tokenizer": tok.state(), "config": asdict(cfg), "samples": {"pretrain": len(pre), "instruct_pairs": len(instruct_pairs), "chat_pairs": len(chat_pairs)}, "loss": {"pretrain": "full causal LM", "instruct": "assistant answer only", "chat": "assistant answer only"}}, indent=2))

    stages = [
        ("pretrain", PretrainDataset(pre, tok, args.block_size), args.pretrain_epochs),
        ("instruct", AssistantPairDataset(instruct_pairs, tok, args.block_size), args.instruct_epochs),
        ("chat", AssistantPairDataset(chat_pairs, tok, args.block_size), args.chat_epochs),
    ]
    history = []
